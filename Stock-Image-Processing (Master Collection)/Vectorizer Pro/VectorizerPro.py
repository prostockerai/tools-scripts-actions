...
class VectorizerProApp:
    def __init__(self, master):
        ...
        self.master.configure(bg="#2b2b2b")  # Dark background color

        # Set up a professional font
        ...
        # "VectorizerPro" gradient title (simulated with two labels, no space)
        ...
        # Copyright Info with clickable link
    ...
    def run_main_process(self):
        ...
        if not files:
            self.log("❌ Error: Input folder is empty.", "#ff6347")
            return
        ...
        with sync_playwright() as p:
            ...
            for i, file in enumerate(files):
                ...
                try:
                    ...
                    page = context.new_page()
                    page.on("download", lambda download: download.save_as(os.path.join(output_folder, download.suggested_filename)))
                    page.goto("https://vectorizer.ai/")
                    page.wait_for_selector("a[href='/pricing']")
                    ...
                    page.wait_for_selector(ok_button_selector, timeout=60000)
                    page.click(ok_button_selector)
                    
                    first_download_link_selector = 'a#App-DownloadLink'
                    page.wait_for_selector(first_download_link_selector, timeout=600000)
                    page.click(first_download_link_selector)
                    
                    page.wait_for_selector('label:has-text("EPS")')
                    page.click('label:has-text("EPS")')
                    page.wait_for_selector('label:has-text("Fine")')
                    page.click('label:has-text("Fine")')
                    
                    final_download_button_selector = '#Options-Submit'
                    page.wait_for_selector(final_download_button_selector)
                    ...
                    self.log(f"❌ Error: {os.path.basename(file_path)} - {e}", "#ff6347")
